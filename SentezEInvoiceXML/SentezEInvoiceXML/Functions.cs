...
namespace SentezXMLDeneme {
    ...
    public static class Functions {
        public const string DEFAULT_CONN_STRING_SQL = @"Server=192.168.1.99; Database=Live; uid=sa; pwd=s; connection timeout=59;";
        ...
        public static DataTable getDataFromSql(string connString, string query, string name = "") {
            ...
            try {
                ...
                //Log uLog = new Log();
                Console.WriteLine("getDataFromSql -> SQL veri alınamadı.", filePath);
        ...
        public static string makeJson(DataSet dat, string textBoxWebUsername, string textBoxWebUserPass) {
            ...
            try {
                ...
                if (userName == "") {
                    Console.WriteLine("textBoxWebUsername Empty!!!", filePath);
                }

                string passWord = textBoxWebUserPass;
                if (passWord == "") {
                    Console.WriteLine("textBoxWebUserPass Empty!!!", filePath);
                }
                string firstOutput = "{\n    \"settings\": {\n      \"user\": \"" + userName + "\",\n      \"pass\": \"" + passWord + "\"\n    },\n    \"products\":";
                string lastOutput = "\n}";
                ...
                Console.WriteLine("makeJson -> JSON oluşturulamadı.", filePath);
                throw;
            }
        ...
        public static string writeToSite(string url1, string json) {
            ...
            try {
                ...
                Console.WriteLine("writeToSite -> Siteye yazamadı.", filePath);
                return "";
                //throw;
            }
        ...
        public static string createUrl4WebOrders(string url, string textBoxDaysAgo, string textBoxWebUsername, string textBoxWebUserPass, bool isUpdateDate = true) {
            ...
            if (days == 0) {
                days = 7;
                Console.WriteLine("textBoxDaysAgo Boş olamaz ->" + days.ToString(), filePath);
            }
            ...
            if (user == "") {
                user = "uzaktanGLR6523";
                Console.WriteLine("textBoxWebUsername Boş olamaz ->" + days.ToString(), filePath);
            }
            string pass = textBoxWebUserPass;
            if (pass == "") {
                pass = "654Dfg654DFgj5645";
                Console.WriteLine("textBoxWebUserPass Boş olamaz ->" + days.ToString(), filePath);
            }
            ...
            try {
                using (SqlConnection connection = new SqlConnection(connString)) {
                    using (SqlCommand command = new SqlCommand()) {
                        ...
                        Console.WriteLine("sendSQLCommand ->" + query, filePath);
                        ...
                        try {
                            ...
                            Console.WriteLine("sendSQLCommand" + recordsAffected.ToString() + " kadar işlem yapıldı!", filePath);
                        }
                        catch (SqlException) {
                            // error here
                            Console.WriteLine("Sql Error", filePath);
                        }
                        finally {
                            connection.Close();
                        }
                ...
                Console.WriteLine("sendSQLCommand -> SQL hatası.", filePath);
            ...
            try {
                using (SqlConnection connection = new SqlConnection(connString)) {
                    using (SqlCommand command = new SqlCommand()) {
                        command.Connection = connection;
                        command.CommandType = CommandType.Text;
                        Console.WriteLine("executeScalar ->" + query, filePath);
                        command.CommandText = query;
                        string result = "";
                        try {
                            connection.Open();
        ...
        public static void insertToSQL(string connString, string tableName, params SqlObj[] sqlObjs) { // send 
            ...
            try {
                using (SqlConnection connection = new SqlConnection(connString)) {
                    using (SqlCommand command = new SqlCommand()) {
                        ...
                        try {
                            ...
                            int recordsAffected = command.ExecuteNonQuery();
                            Console.WriteLine("insertToSQL " + recordsAffected.ToString() + " kadar işlem yapıldı!", filePath);
                        }
                ...
                Console.WriteLine("insertToSql -> SQL hatası.", filePath);
        ...
        public static void updateSQL(string connString, string tableName, string conditions, params SqlObj[] sqlObjs) {
            ...
            try {
                using (SqlConnection connection = new SqlConnection(connString)) {
                    using (SqlCommand command = new SqlCommand()) {
                        ...
                        try {
                            connection.Open();
                            int recordsAffected = command.ExecuteNonQuery();
                            Console.WriteLine("updateSQL " + recordsAffected.ToString() + " kadar işlem yapıldı!", filePath);
                        }
                        catch (SqlException) {
                            // error here
                            Console.WriteLine("Sql Error", filePath);
                ...
                Console.WriteLine("updateSql -> SQL hatası.", filePath);
                throw;
            }
        }
        ...
        public static string getPicture(string InventoryId) {
            ...
            using (var conn = new SqlConnection(Functions.DEFAULT_CONN_STRING_SQL)) {
                using (var cmd = new SqlCommand()) {
                    cmd.Connection = conn;
                    cmd.CommandType = CommandType.Text;
                    //Console.WriteLine("sendSQLCommand ->" + queryCountRecId);
                    //Clipboard.SetText(queryCountRecId);
                    ...
                    Console.WriteLine("Image Base64 Copied to clipboard!", filePath);
                }
            }
            return "";
        }
