...
#pragma warning restore 0649
        ...
        private string m_procedureResult = "";
        ...
        public SqlObj(string sqlConn) {
            // SqlObj.DEFAULT_CONN_STRING
            initalizeSqlConnection(sqlConn);
        }
        ~SqlObj() {
        ...
        public void insert(string tableName, List<SqlPair> sqlPairs, bool sendCommand = false) {
            ...
            */
            ...
            foreach (var SqlPair in sqlPairs) {
                 ...
                 */
                m_command.Parameters.AddWithValue("@" + SqlPair.colName, SqlPair.value);
            }
            if (sendCommand) {
                executeCommand(EnumSqlCommands.Insert);
            }
        ...
        /// Procedure
        public void procedure(string procedureName, List<SqlPair> pairs, bool sendCommand = false) {
            /*UPDATE table_name SET [column_name1= value1,...column_nameN = valueN] [WHERE expression2]
              UPDATE students SET User_Name = 'Sonoo' WHERE Student_Id = '3' 
              "UPDATE Student SET Address = @add, City = @cit Where FirstName = @fn AND LastName = @ln"
              -> command.Parameters.AddWithValue("@ln", lastName);
             */
            m_query = procedureName;
            m_command.CommandType = CommandType.StoredProcedure;
            m_command.CommandText = m_query;
            foreach (var pair in pairs) {
                m_command.Parameters.AddWithValue("@" + pair.colName, pair.value);
            }
            if (sendCommand) {
                executeCommand(EnumSqlCommands.Procedure);
            }
        }
        ...
        public void truncate(string tableName, bool sendCommand = false) {
            ...
            //cmd.ExecuteNonQuery();
            if (sendCommand) {
                executeCommand(EnumSqlCommands.Truncate);
            }
        }
        /// execute command
                ...
                using (m_command) {
                    try {
                        ...
                        switch (enumSqlCommand) {
                            ...
                            case EnumSqlCommands.Truncate:
                                m_recordsAffected = 0;
                                m_recordsAffected = m_command.ExecuteNonQuery();
                                //MessageBox.Show(m_recordsAffected + " kadar işlem yapıldı.");
                                break;
                            case EnumSqlCommands.Procedure:
                                m_recordsAffected = 0;
                                m_procedureResult = "";

                                var returnParameter = m_command.Parameters.Add("@ReturnVal", SqlDbType.VarChar);
                                returnParameter.Direction = ParameterDirection.ReturnValue;

                                m_recordsAffected = m_command.ExecuteNonQuery();
                                try {
                                    m_procedureResult = returnParameter.Value.ToString();
                                }
                                catch (SqlException ex) {
