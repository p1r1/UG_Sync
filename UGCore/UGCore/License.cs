...
namespace UGCore {
    public class License {
        ...
        public static bool isLicenseValid() {
            /// get key and serial
            string filePath = AppDomain.CurrentDomain.BaseDirectory + "FrameworkMetro";
            if (!File.Exists(filePath)) {
                MessageBox.Show("Ürün anahtarı yok!"); 
                return false;
            }
            
            UserRecord userRecord = XmlHelper.FromXmlFile<UserRecord>(filePath);
            SerialRecord serialRecord = new SerialRecord();
            serialRecord =  userRecord.serialRecord;

            if (new FileInfo(filePath).Length != 0 && (Object.ReferenceEquals(null, serialRecord) ||
                serialRecord.publicKey.Length == 0 || serialRecord.serial.Length == 0)) {
                try {
                ...
                MessageBox.Show("Ürün anahtarı bozulmuş!");
                return false;
            }

            //Console.WriteLine($@"publicKey: {serialRecord.publicKey} \n seialKey: {serialRecord.serial}");

            string publicKey = serialRecord.publicKey;
            string licenseSeriazlizedText = serialRecord.serial;

            /// if serialKey or publicKey is empty
            if (licenseSeriazlizedText == string.Empty || publicKey == string.Empty) {
                Console.WriteLine("Ürün anahtarı girilmemiş!");
            ...
            if (isExpired == false) {
                ...
                //Console.WriteLine("Ürün anahtarının süresi dolmuş!");
            ...
            if (verifyLicenseText != licenseSeriazlizedText && isExpired == false) {
                ...
                //Console.WriteLine("Ürün anahtarının doğrulanamadı!");
        ...
        private Tuple<bool, string> verifyLicense(string publicKey, string licenseText) {
            try {
        ...
        private bool verifyDigits(string s) {
            return true;
        }
