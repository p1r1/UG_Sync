...
namespace UGDiscordBot {
    public class OrderReceipt {
        ...
        public class Customer {
            ...
            public string m_ordCity { get; }
            public string m_ordTown { get; }
            public List<Product> m_productList { get; set; }

            public Customer(string name, string ordCode, string ordPaymentType, string ordDate, string ordTotalPrice, /*string ordPercentage,*/ string ordChangeDate, string ordCargo, string city, string town) {
                m_name = name;
                m_ordCode = ordCode;
        ...
        public List<Customer> checkOrders(string url, string daysAgo, string webUsername, string webUserPass, string ordStatus = "1") {
            /// ///
            ...
            if (!Object.ReferenceEquals(null, webOrders.m_data.orders)) {
                foreach (var ordr in webOrders.m_data.orders) {
                    if (!Object.ReferenceEquals(null, ordr.ord_status)) {
                        if (ordr.ord_status.ToString() == ordStatus) {
                            ...
                            string cOrdCity = !Object.ReferenceEquals(null, ordr.ord_shipping_city_id) ? ordr.ord_shipping_city_id.ToString() : "";
                            string cOrdTown = !Object.ReferenceEquals(null, ordr.ord_shipping_town_id) ? ordr.ord_shipping_town_id.ToString() : "";

                            Customer c = new Customer(cAccountName, cSpecialCode, cPaymentType, cOrdDate, cGrandTotal, cUpdateDate, cOrdCargo, cOrdCity, cOrdTown);
                            List<Product> pList = new List<Product>();
                            if (!Object.ReferenceEquals(null, ordr.products)) {
                                foreach (var item in ordr.products) {
                                    if (Object.ReferenceEquals(null, item.orp_prd_name) ||
                                        item.orp_prd_name.ToString() == "PTT" || 
                                        item.orp_prd_name.ToString() == "Aras Kargo" || 
                                        item.orp_prd_name.ToString() == "Yurtiçi Kargo") {
                                        continue;
                                    }
